...
const ActivityTracker = (() => {
    ...
    const vd = new Date(1628630166375);
    ...
    const logmsgbox = ({ c: c = "chat message", t: t = "title", btn: b = "buttons", send: send = true, sendas: sas = "API", wto: wto = "", type: type = "normal" }) => {
        let tbl = msgtable.replace("__bg__", rowbg[0]);
        let hdr = msg1header.replace("__bg__", headerbg[type]).replace("__cell1__", t).replace('__colspan__', '4');
        let row = c;
        let btn = b !== "buttons" ? msg1row.replace("__bg__", rowbg[0]).replace("__cell1__", b).replace("__row-css__", "text-align:right;margin:4px 4px 8px;") : "";
        let msg = tbl.replace("__TABLE-ROWS__", hdr + row + btn);
        if (wto) msg = `/w "${wto}" ${msg}`;
    ...
    const handleInput = (msg) => {
        ...
        */
        ...
        if (/^!track(\s*$|\s+--current|\s+--config)/.test(msg.content)) {
            ...
            let disabledquery = `?{Choose Event|${Object.keys(state[apiproject].config.events).filter(k => !state[apiproject].config.events[k]).map(k => k.replace(/:/g, '&#58;')).join('|')}}`;
