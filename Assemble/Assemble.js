...
const Assemble = (() => { // eslint-disable-line no-unused-vars
    ...
    const handleInput = (msg) => {
        ...
        if (argObj.char && argObj.char.id) {
            ...
            if (!explicitDelim && ['-', '+'].includes(argObj.mode) && attr.get('max').length && /\(delim:([^)]+)\)/.test(attr.get('max'))) {
            ...
            if (data.length) {
                ...
                msgbox({
                    ...
                    msg: `${argObj.mode === '+' ? 'Removed' :
                        argObj.mode === '-' ? 'Added' : 'Overwrote with'} ${data.length} entries. Current contents are shown below.` +
        ...
        if (argObj.menu && argObj.menu.id) {
            ...
            let hdr = html.tr(
                html.th('ATTR', localCSS.textleft) + html.th('ADD') + html.th('REM') + html.th('OVER')
            );
