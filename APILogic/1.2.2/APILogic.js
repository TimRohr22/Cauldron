...
const APILogic = (() => {
    ...
    const vd = new Date(1615394348037);
    const versionInfo = () => {
    ...
    const groupopenrx = /^\s*(?<negation>!?)\s*\(\s*/,
        ...
        sheetitemrx = /^(?<negation>!?)(?<type>(?:@|%))(?<operation>[^\s@%|]*)\|(?<character>[^|]+?)\|(?<item>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}\s*(?=!=|!~|>=|<=|[=~><]|&&|\|\||\)|})/i,
        rptgitemrx = /^(?<negation>!?)(?<type>(?:\*))(?<operation>[^\s*|]*)\|(?<character>[^|]+?)\|(?<section>[^\s|]+)\|\[\s*(?<pattern>.+?)\s*]\s*\|(?<valuesuffix>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}\s*(?=!=|!~|>=|<=|[=~><]|&&|\|\||\)|})/i,
        sheetitem_standalonerx = /^(?<type>(?:@|%))(?<operation>[^\s@%|]*)\|(?<character>[^|]+?)\|(?<item>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}\s*/i,
        rptgitem_standalonerx = /^(?<type>(?:\*))(?<operation>[^\s*|]*)\|(?<character>[^|]+?)\|(?<section>[^\s|]+)\|\[\s*(?<pattern>.+?)\s*]\s*\|(?<valuesuffix>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}\s*/i,
        sheetitem_replacerx = /(?<!!)(?<type>(?:@|%))(?<operation>[^\s@%|]*)\|(?<character>[^|]+?)\|(?<item>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}/g,
        rptgitem_replacerx = /(?<!!)(?<type>(?:\*))(?<operation>[^\s*|]*)\|(?<character>[^|]+?)\|(?<section>[^\s|]+)\|\[\s*(?<pattern>.+?)\s*]\s*\|(?<valuesuffix>[^)\]\s=~<>!&\|]+)(?:\|\[(?<default>.*?)]){0,1}/,
        comprx = /^(?<operator>(?:>=|<=|~|!~|=|!=|<|>))\s*/,
    ...
    // MULE REGEXES =========================================
    const varrx = /^((?:(?:\d+)-(?:\d+)|(?:!=|>=|<=|>|<)(?:\d+))|[^\s]+?)=(.+)$/,
        getrx = /get\.([^.]+\.[^\s.]+?\.[^\s\W.]+|[^\s.]+?\.[^\s\W.]+|[^\s\W]+)(\||\b)/gmi,
    ...
    const mulegetter = (preserved) => {
        ...
        if (mulerx.test(preserved.content)) {
            ...
            characters = playerIsGM(preserved.playerid) ? characters : characters.filter(c => c.get('controlledby').split(',').includes(preserved.playerid));
        ...
        preserved.content = preserved.content.replace(getrx, (m, gvar) => {
            ...
            return retval || `${fullgvar}`; // remove the `get.` so we don't trigger infinite loops of processing
        });
    };
    ...
    const ifTreeParser = (preserved) => {
        ...
        const getConditionToken = (c) => {
            let logcolor = 'white';
            let index = 0;
            let firstargres = getfirst(c.cmd, comptm, sheetitemtm, rptgitemtm, texttm);
            if (firstargres) {
                ...
                if (compres) {
                    ...
                    let secondargres = getfirst(c.cmd.slice(index), sheetitemtm, rptgitemtm, groupendtm, ifendtm, operatortm, texttm);
    ...
    //      LOGGING
    // ==================================================
    const handleLogging = (preserved) => {
        let logrx = /{\s*&\s*log\s*(?<setting>(?:on|off)?)\s*}/ig;
        preserved.statelog = state[apiproject].logging;
        preserved.loclog = 'none';
        preserved.content = preserved.content.replace(logrx, ((r, g1) => {
            preserved.loclog = g1 || 'temp';
            return '';
        }));
        if (['on', 'temp'].includes(preserved.loclog)) state[apiproject].logging = true;
        if (preserved.loclog === 'off') state[apiproject].logging = false;
    };
        ...
        if (Object.keys(preservedMsgObj).length && trigrx.test(msg.content)) { // check all active apitriggers in play
            ...
            if (msg.inlinerolls) {
                ...
                if (!testConstructs(msg.content) && !testSecondaryPass(preserved, msg)) { // we're on our way out of the script, format everything and release message
                    ...
                    if (preserved.loclog === '') state[apiproject].logging = preserved.statelog;
