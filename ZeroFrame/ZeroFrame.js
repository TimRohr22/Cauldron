...
const ZeroFrame = (() => {
    ...
    const vd = new Date(1618407837440);
    ...
    const buildConfig = (msg) => {
        ...
        let rows = looporder.reduce((m, v) => {
            return m + msgconfigcontent
                .replace('__PRIORITY__', v.priority)
                .replace('__SCRIPTNAME__', v.name)
                .replace('__ALIASES__', v.handles.join(', '));
        }, '');
    ...
    const setOrder = (msg, preservedstate) => {
        let orderrx = /{&\s*0\s+([^}]+?)\s*}/g;
        msg.content = msg.content.replace(orderrx, (m, list) => {
            let order = list
                .split(/\s+/)
    ...
    const testConstructs = (c) => {
        if (/^!0\s+(cfg|config)/.test(c)) return 'showconfig';
                ...
                case 'runconfig':
                    zfconfig.exec(msg.content).groups.scripts
                        ...
                        .forEach(c => {
                            ...
                            else notfound.push(c[0]);
                        });
                    buildConfig(msg);
                    if (notfound.length) {
                        // TO DO: replace this with built-in reporting
                        msgbox({ c:`Cannot change the priority for a non-existent or inactive script. No scripts found for the following handles:<br>${notfound.join('<br>')}`, wto: msg.who });
                    }
                    break;
                case 'help':
