...
const ShowButtons = (() => { // eslint-disable-line no-unused-vars
    ...
    const msgbox = ({
        ...
        if (title) title = html.div(html.div(html.img(apilogoalt, 'ButtonShow Logo', localCSS.logoimg), localCSS.msgheaderlogodiv) + html.div(title, localCSS.msgheadercontent), {});
    ...
    const handleInput = (msg) => {
        ...
        Object.keys(argObj).forEach(k => {
            argObj[k].forEach(a => {
                let character = getChar(a.char, msg.playerid);
    ...
    const handleConfig = msg => {
        ...
        if (/^!showbuttons?config\s+[^\s]/.test(msg.content)) {
            ...
            message = `<b>ShowButtons</b> is currently configured as follows:<br>${cfgTrack.report}`;
