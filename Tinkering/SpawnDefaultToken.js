...
const SpawnDefaultToken = (() => {
    ...
    const processCommands = function(data, args) {
        let retVal = { tokens: [], errors: [] };        //array of potential error messages to pass back to main handleInput funtion
        ...
        try {
                ...
                args.forEach((arg) => {
                    ...
                    switch(option) {
                        ...
                        case "targs":
                            ...
                            retVal.errors.push('Cannot re-use the api chat button');
                            return retVal;
                            break;
                        case "targets":
                        ...
                        case 'force':
                            if (_.contains(['true', 'yes', '1'], param.toLowerCase())) {
                                ...
                                retVal.errors.push('Invalid force to square argument (\"' + param + '\"). Choose from: (' + data.validPlacements + ')');
                                return retVal;
                            }
                            break;
                        case "offset":
                        ...
                        case "tokenname":
                            ...
                            break;
                        default:
                            retVal.errors.push('Unexpected argument identifier (' + option + '). Choose from: (' + data.validArgs + ')');
                            break;    
                    }
                }); //end forEach arg
            } else {
                retVal.errors.push('No arguments supplied. Format is \"!Spawn --Command|Value\"');
                return retVal;
            }
            ...
            if (data.spawnName === "") {
                retVal.errors.push('No spawn target identified. Argument \"spawn|characterName\" required');;
            }
            ...
            if ( _.contains(['stack', 'row', 'col', 'column', 'surround'], data.placement.toLowerCase()) ) {
                //Good, no additional info req'd
                    ...
                    if ( !data.placement.match(/(\d+)/) ) {
                        retVal.errors.push('Invalid grid row length supplied (\"' + data.placement + '\"). Format is --placement|grid #');
                    } else {        
                        //good grid #
                    ...
                    if ( !data.placement.match(/(\d+)/) ) {
                        retVal.errors.push('Invalid burst radius supplied (\"' + data.placement + '\"). Format is --placement|burst #');
                    } else {        
                        //good burst #
                    ...
                    //burst case    --check for number
                    if ( !data.placement.match(/(\d+)/) ) {
                        retVal.errors.push('Invalid cross radius supplied (\"' + data.placement + '\"). Format is --placement|cross #');
                    } else {        
                        //good burst #
                    ...
                    if ( !data.placement.match(/(\d+)/) ) {
                        retVal.errors.push('Invalid random grid row length supplied (\"' + data.placement + '\"). Format is --placement|random #');
                        ...
                        retVal.errors.push('Input qty (\"' + data.qty + '\") exceeds the number of available grid squares(\"'+ numSquares + '\"). Consider increasing the grid size or reducing qty.');
                    } else {        
                        //good grid #
                        data.gridCols = data.placement.match(/(\d+)/)[0];   //use first number found for gridCols
                ...
                retVal.errors.push('Invalid placement argument (\"' + data.placement + '\"). Choose from: (' + data.validPlacements + ')');
            }
            
            //Check for valid offset X/Y (numeric)
            if (isNaN(data.offsetX) || isNaN(data.offsetY)) {
                retVal.errors.push('Non-numeric offset detected. Format is \"--offset|#,#\" in Squares');
            } else if (data.offsetX > 50*70 || data.offsetY > 50*70) {
                //In case the offset was entered in pixels
                retVal.errors.push('Offset out of range. Format is \"--offset|#,#\" in Squares (Max 50)');
            }
            
            //size must be "#,#""
            if (isNaN(data.sizeX) || isNaN(data.sizeY) || data.sizeX === null || data.sizeY === null) {
                retVal.errors.push('Non-numeric size detected. Format is \"--size|#,#\"');
            }
            
            //light must be "#,#""
            if (isNaN(data.lightRad) || isNaN(data.lightDim) || data.lightRad === null || data.lightDim === null) {
                retVal.errors.push('Non-numeric light radius detected. Format is \"--size|#,#\" \(bright, dim\)');
            }
            
            //Numeric qty between 1 and 20 required
            if (isNaN(data.qty)) {
                retVal.errors.push('Non-numeric qty detected. Format is \"--qty|#\"');
            } /* else if ( data.qty <  1 || data.qty > 20 ) {
                retVal.errors.push('Input qty out of range. Must be between 1 and 20.');
            }
            */
            ...
            if (data.fx !== '') {
                ...
                if (fx.length !== 2) {
                    retVal.errors.push('Invalid FX format. Format is --fx|type-color');
                } else if (fxModes.indexOf(fx[0]) === -1 ) {
                    retVal.errors.push('Invalid FX type requested. Supported types are ' + fxModes.join(','));
                } else if (fxColors.indexOf(fx[1]) === -1 ) {
                    retVal.errors.push('Invalid FX color requested. Supported colors are ' + fxColors.join(','));
                }
            }
            ...
            if (data.expandIterations !== 0) {
                if (isNaN(data.expandIterations)) {
                    retVal.errors.push('Non-numeric animation iterations detected. Format is \"--expand|#,#\" \(iterations, delay\)');
                }
                if (isNaN(data.expandDelay)) {
                    retVal.errors.push('Non-numeric animation delay detected. Format is \"--expand|#,#\" \(iterations, delay\)');
                }
            }
            ...
            if (!isNumber(data.angle)) {
                if(!_.contains(['random','rand'], data.angle.toLowerCase())) {
                    retVal.errors.push('Invalid rotation detected. Format is \"--rotation|# or rand/random\"');
                } else {
            ...
            if (data.userSpecifiedLayer) {
                if ( data.spawnLayer.match(/obj/i) || data.spawnLayer.match(/tok/i) ) {
                    ...
                    retVal.errors.push('Invalid layer requested. Valid layers are \"object(s)\", \"token\", \"tok\", \"gm\",\"map\"');
                }
            }
            ...
            syncSpawn(DefaultTokens.getToken(spawnObj.id));
    ...
    const handleInput = function(msg) {
        try {
            if ((msg.type == "api" && msg.content.indexOf("!Spawn") === 0) || (msg.eval && /^!spawn/.test(msg.content))) {
    ...
    const spawn = (m, s) => handleInput(m, s);
    on("ready",() => {
        checkInstall();
        registerEventHandlers();
        try {
            Plugger.RegisterRule(spawn);
        } catch (error) {
            log(error);
        }
    });
...
const DefaultTokens = (() => {
    ...
    const handleDefaultToken = (char, prev) => {
    ...
    const buildOrig = () => {
        findObjs({ type: 'character' }).forEach(c => {
            handleDefaultToken(c);
        });
    };

    const getToken = (id) => { return tokens[id]; };

    const registerEventHandlers = () => {
        on('change:character:_defaulttoken', handleDefaultToken)
    };
