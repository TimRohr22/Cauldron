                    ...
                    obj: Object.keys(state)
                        .map(k => {
                            return {
                                ...
                                button: [
                                    { label: html.tip('&ast;', 'Archive and Delete'), command: `!about-archive --${k}`, css: combineCSS(localCSS.buttoncss, localCSS.warningbutton) },
                                    { label: html.tip('F', 'Archive'), command: `!about-archive --${k}`, css: localCSS.buttoncss },
                                    { label: html.tip('s', 'View'), command: `!about --state.${k}`, css: localCSS.buttoncss }
                                ]
                            }
                        })
                });
                        ...
                        if (!o || o.fail) failHandler(wto, (o || { reason: `notfound${!manageState.get('playersCanIDs') && !playerIsGM(msg.playerid) ? 'id' : ''}` }).reason);
                            ...
                            rows = Object.keys(o.bytype).map(k => {
                                return html.tr(html.td(k.toUpperCase()) + html.td('', { width: '50px' }), { 'border-bottom': '1px solid #222d3a', 'font-size': '14px', 'font-weight': 'bold' }) + o.bytype[k].map(item => {
                                    let btncell = '';
                                    if (!Array.isArray(item.button)) {
                                        btncell = Messenger.Button({ type: '!', elem: item.button || '!about --' + item._id, label: 's', css: localCSS.buttoncss });
                                    } else {
                                        btncell = item.button.map(b => Messenger.Button({ type: '!', elem: b.command, label: b.label, css: b.css || localCSS.buttoncss })).join('');
                                    }
                                    return `${html.tr(html.td(getTipFromObjForID(item)) + html.td(btncell, { width: '100px', 'text-align': 'right' }))}`;
                                }).join('');
                            }).join('');
